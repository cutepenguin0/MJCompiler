...
public class CodeGenerator extends VisitorAdaptor {
	...
	public void visit(MaxCalc matrix) {
	    ...
	    Code.load(matrix.getDesignator().obj); //2 arr[1] 2 2 arr 
	    Code.put(Code.arraylength); //2 arr[1] 2 2 7
	    // Compare index with array size (i <= size)
//	    Code.put(Code.dup_x1); // arr[1] 2  2 7 
	    Code.putFalseJump(Code.lt, 0); // if i >= size, exit loop EXIT LOOP
	    int loopExit = Code.pc - 2; // NE ZNAM ISPRAVITI
	    ...
	    Code.putFalseJump(Code.le, 0); // if max > value, skip update
